...
class Api{
    companion object{suspend inline fun <reified T> get(url: String, params: Map<String, String?>? = null): List<T> {
        ...
        return if (response.status == HttpStatusCode.OK)
            response.body<PaggedArray<T>>().items
        else
            emptyList()
    }

        suspend inline fun <reified T> post(url: String, params: Map<String, String?>? = null, body: T) =
            client.post(url) {
                url {
                    params?.forEach { (name, value) ->
                        value?.apply { parameters.append(name, value) }
        ...
        suspend inline fun <reified T> patch(url: String, params: Map<String, String?>? = null, body: T) =
            client.patch(url) {
                url {
                    params?.forEach { (name, value) ->
                        value?.apply { parameters.append(name, value) }
