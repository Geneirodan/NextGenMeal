...
public class OrderService : BaseService, IOrderService
{
    ...
    private readonly int deleteTimeout;
    public OrderService(
        ...
        IOptions<PaginationOptions> paginationOptions) : base(paginationOptions)
    {
        ...
        deleteTimeout = orderOptions.Value.DeleteTimeout;
    }
    ...
    public async Task<Result<OrderModel>> AddAsync(ClaimsPrincipal principal, OrderModel model)
    {
        ...
        if (!b) return Result.Fail(Errors.InvalidDishes);
    ...
    public async Task<Result> DeleteAsync(ClaimsPrincipal principal, int id)
    {
        ...
        if (user is Customer)
        {
            var timeout = TimeSpan.FromMinutes(deleteTimeout);
    ...
    public Result<List<OrderDishModel>> GetOptimal(int cateringId, decimal maxPrice, Dictionary<string, int> types)
    {
        try
        {
            ...
            var tagDishes = types.Select(type => new TagDish(type)
                {
            ...
            {
                var dish = i.Dishes.First();
                return new OrderDishModel
                {
                    Quantity = i.Type.Value,
                    DishId = dish.Id,
                    Dish = dish.Adapt<DishModel>()
                };
            }).ToList();
    ...
    public async Task<Result> ReceiveAsync(int id) =>
    ...
    private async Task<Result> SetStatus(int id, string oldStatus, string newStatus)
    {
        ...
        if (order.IsBox && newStatus == OrderStatuses.Received)
        {
            ...
            var message = new MqttApplicationMessageBuilder()
                .WithTopic($"{mqttOptions.Prefix}/{terminal.SerialNumber}")
